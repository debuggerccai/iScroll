<template>
  ...
  >
    <view
      class="content"
      :id="contentId"
      :style="{transitionTimingFunction, transitionDuration, transform}"
    >
      <!-- 下拉刷新 -->
...
export default {
  ...
  async mounted() {
    ...
    // this.$refs['scroll-wrapper'].$el.addEventListener('wheel', function(e) {
    // 	console.log(e)
    // })
  },
  methods: {
...
.spinner-holder .dot {
  ...
  opacity: 0.5;
  border-radius: 20px;
  animation: animation 1s linear 0.3s infinite;
}

.spinner-holder .dot:nth-child(2) {
  animation: animation 1s linear 0.2s infinite;
}

.spinner-holder .dot:nth-child(3) {
  animation: animation 1s linear 0.1s infinite;
}
